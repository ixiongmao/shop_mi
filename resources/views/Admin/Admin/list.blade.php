                            ...
                            <tbody>
                              @foreach ($data as $k => $v)
                                <tr>
                                    ...
                                    <td><a class="btn btn-outline btn-default btn-sm" href="/admin/admin/record/{{ $v->id }}" title"点击查看员工登录操作记录">{{ long2ip($v->a_ip) }} &nbsp;点击查看员工登录记录</button>
                                    </td>
                                    <td>{{ date('Y-m-d H:i:s',$v->a_time) }}</td>
                                    <td><a href="/admin/admin/edit/{{ $v->id }}" class="btn btn-primary btn-sm">修改</a><!--  |
                                        <a href="/admin/admin/destroy/{{ $v->id }}" class="btn btn-danger btn-sm" onclick="return confirm('删除后放入回收站，你确定要删除吗？(可在回收站恢复)')">删除</a>-->
                                    </td>
                                </tr>
                                @endforeach
                              </tbody>
