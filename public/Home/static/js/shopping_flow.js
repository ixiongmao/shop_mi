...
{
  /* 预售*/
  var is_presale = document.getElementById("is_presale").value;
  ...
  var now = new Date();
  Ajax.call('flow.php?step=select_shipping', 'shipping=' + obj.value+'&is_presale='+is_presale, orderShippingSelectedResponse, 'GET', 'JSON');
}
...
function selectPayment(obj)
{
  ...
  Ajax.call('flow.php?step=select_payment', 'payment=' + obj.value+'&is_presale='+is_presale, orderSelectedResponse, 'GET', 'JSON');
}
/* *
 * 团购购物流程 --> 改变配送方式
 */
...
function changeSurplusService(val)
{
    if (selectedSurplus == val)
    {
        return;
    ...
    Ajax.call('flow.php?step=change_surplus_service', 'surplus=' + val, changeSurplusResponse, 'GET', 'JSON');
}
